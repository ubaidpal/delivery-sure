...
class TransactionsController extends Controller
{
    ...
    public function index() {
        ...
        $key           = Input::get('key');
        $data[ 'key' ] = $key;
        $query         = Transaction::select(['user_id'])->groupBy('user_id')->with('user')->orderBy('id', 'DESC');
        ...
        if(!empty($key)) {
            $users = User::where('display_name', 'like', "%$key%")
        ...
        $data[ 'seller' ] = [];
        $strObj           = new ManagementRepository();
        foreach ($transactions as $item) {
            $last_month_sales       = Transaction::where('user_id', $item->user_id)
                                                      ->select(DB::raw('SUM(amount) AS total_price'))
                                                      ->where(DB::raw('DATE(created_at)'), '>', date('Y-m-d', strtotime('-30 days')))
                                                      ->first();
            $item->total_orders     = Transaction::where('user_id', $item->user_id)
        ...
        $data[ 'transactionCollapse' ] = 'collapse in';
        $data[ 'transColor' ] = 'color:#fff';
    ...
    public function withddrawalRequests() {
        ...
        $query  = Withdrawal::orderBy('id', 'DESC');
        if(!empty($term)) {
            $users = User::where('display_name', 'like', "%$term%")
                         //->where('userable_type', 'App\Brand')
                         ->lists('id', 'id')->toArray();
            if(!empty($users)) {
        ...
        $data[ 'status' ]      = $status;
        $data['WithdrawalCollapse'] = 'collapse in';
        $data['WithdrawalColor'] = 'color:#fff';

        return view('admin.transactions.storeWithdrawlRequests', $data);
    }

    public function viewPaymentMethodDetails(Request $request) {
        $data   = [];
        $method = WithdrawalMethod::where('id', $request->method_id)->first();

        if($method->method == 'bank') {
            $bank           = BankAccount::where('store_withdrawal_method_id', $request->method_id)->first();
            $data[ 'bank' ] = $bank;
        }
        $data[ 'method' ] = $method;
    ...
    public function chagePaymentStatus(Request $request) {
        $data                 = ['withdrawal_id' => $request->withdrawal_id];
    ...
    public function startPaymentProcess($withdrawal_id) {
        $withdrawal = Withdrawal::where('id', $withdrawal_id)->select(['id', 'status'])->first();
    ...
    public function savePaymentinfo($withdrawal_id) {
        ...
        if(\Request::hasFile('attachment') && \Request::file('attachment')->isValid()) {
            $file = $this->storeFile(\Request::file('attachment'), 'withdrawal');

            $withdrawal->deposit_attachment = $file;
        }
    ...
    public function chageClaimPaymentStatus($withdrawal_id) {
        ...
        if(!empty($withdrawal->deposit_attachment)) {

            $withdrawal->attachment_path = url('admin/getAttachment/deposit_slip/' . $withdrawal->deposit_attachment);
