...
use Repositories\Entities\Message as MessageEntity;
use Repositories\Entities\Conversation as ConversationEntity;
...
use UserDetail;
    ...
    public function get_conversations($user_id, $convId = FALSE) {
        ...
        if($total > 0) {
            ...
            $messages = $this->getConversationMessages($convId, $user_id, FALSE);
    ...
    public function save_message($data, $user_id) {
        ...
        if(isset($data[ 'conv_id' ]) && !empty($data[ 'conv_id' ])) {
            $conv_id = $this->decodeId($data[ 'conv_id' ]);
            return $this->addMessageToConversation($conv_id, $user_id, $data);
        } else {
            //If more
            if(isset($data[ 'receiver_id' ])) {
                $data[ 'receiver' ] = $this->decodeId($data[ 'receiver_id' ]);
            }
    ...
    public function createConversation($members, $user_id, $type = NULL, $orderId = NULL) {
        if(is_null($type)) {
            if(count($members) > 2) {
                $type = self::GROUP;
            } else {
    ...
    public function getConversationByTowUsers($user_id, $receiver_id) {
        $results = \DB::select(/** @lang MySQL */
            "SELECT * FROM conv_users, conversations as co WHERE co.id=conv_id and co.type= '" . self::COUPLE . "' and conv_users.conv_id IN(SELECT conv_id FROM conv_users cu WHERE cu.user_id=$user_id OR cu.user_id=$receiver_id group by cu.conv_id HAVING COUNT(cu.conv_id) = 2) group BY conv_id HAVING COUNT(conv_id)  =  2");
    ...
    public function getConversationByOrderId($user_id, $receiver_id, $orderId) {
        //$conv = Conversation::whereOrderId($orderId)->first();
        $results = \DB::select(/** @lang MySQL */
            "SELECT * FROM conv_users, conversations as co WHERE co.id=conv_id and co.order_id = $orderId and co.type= '" . self::COUPLE . "' and conv_users.conv_id IN(SELECT conv_id FROM conv_users cu WHERE cu.user_id=$user_id OR cu.user_id=$receiver_id group by cu.conv_id HAVING COUNT(cu.conv_id) = 2) group BY conv_id HAVING COUNT(conv_id)  =  2");
    ...
    public function getConvMessages($conv_id, $page, $order = 'DESC') {
        ...
        $data = Message::where('conv_id', $conv_id)
            ->take($per_page)
            ->skip($start_point)
            ->orderBy('id', $order)
            ->get()
    ...
    public function messages_detail($data) {
        ...
        foreach ($data as $row) {
            if($row['content'] != '?-empty-?'){
                if(!$this->is_api()){
                    $row[ 'sender_detail' ] = $this->userDetail($users[ $row[ 'sender_id' ] ]);
                }
    ...
    public function get_user_api_detail($user_id)
    {
        $data = '';
        $users = User::select('id','display_name', 'profile_photo_url', 'rating', 'username')
            ->where('id', $user_id)
            ->first();
       $data = (object) array( 'id' => $users->id ,'display_name' => $users->display_name ,'profile_photo_url' => getImage($users->profile_photo_url), 'rating' => $users->rating ,'username' => $users->username );
        return $data;
    }
    ...
    public function addMessageToConversation($conv_id, $user_id, $data) {
        ...
        $message->file_id   = (isset($data[ 'file_id' ])  && !empty($data[ 'file_id' ]) ? $data[ 'file_id'] : NULL );
        ...
        foreach ($usersInConv as $userInConv) {
            ...
            if($userInConv == $user_id) {
                ...
                $messageStatus->status = $data['body'] == '?-empty-?'? self::READ: self::UNREAD;
    ...
    public function getUserConversations($user_id) {
        ...
        foreach ($convs as $conv) {
            ...
            $conversation->setTitle($conv->title);
            $conversation->setConvFor($conv->conv_for);
    ...
    public function getConversations($user_id) {
        return \DB::select('
            SELECT msg.conv_id as conv_id,conv.title,conv.conv_for, msg.created_at, msg.id "msgId", msg.file_id,msg.content, mst.status, mst.self, users.id "userId"
            FROM messages msg
            INNER JOIN (
            ...
            INNER JOIN  conversations conv ON conv.id=msg.conv_id
    ...
    public function is_conv_open($conv_id) {
        ...
        if($conv->conv_for == 'dispute' && $conv->status == 0 ) {
            return ['closed' => 1, 'msg'=>'Conversation is closed. You cannot send messages'];
        }elseif($conv->conv_for == 'admin'){
            return ['closed' => 1, 'msg'=>'You cannot send messages in this conversation'];
    ...
    private function get_attachment($file_id) {
        return Attachment::whereFileId($file_id)->first();
    }
    ...
    public function decodeId($id) {
        return \Hashids::connection('message')->decode($id)[ 0 ];
    }
